...
(stands for **Framework for Runtime Aggregation of Modularized Entities**). On top of learning
...
**Struct** in Rust is a useful construct to help store data that have things in common.
...
for storage reads and writes so our runtime can perform less read/writes to update
...
:::note
If you wonder what the above mean in Rust, the first line is to define a type alias `AccountOf<T>` which is just a shorthand pointing to the associated type `AccountId` of trait `frame_system::Config` that the generic type `T` is required to be bound of.
...
:::note
We use `Option<BalanceOf<T>>` instead of `BalanceOf<T>` for `price` field, so a Kitty price can be set to `None` to indicate the Kitty is not for sale.
:::
