...
:::note
This tool is designed to be used for testing _before_ going to production.
:::
...
:-------------------------:|:-------------------------:
![image](/assets/try-runtime-ext-1.png)  |  ![image](/assets/try-runtime-ext-2.png)
...
:::tip How-to integrate try-runtime
Refer to this how-to guide on integrating `try-runtime` to your project.
:::
...
```
:::info Examples
See how the Executive pallet implements the `try_runtime_upgrade` method [here][executive-example-frame] to aggregate all `OnRuntimeUpgrade` calls from a runtime and return a `Weight`, which could be used for testing. 
::: 
...
```
## Examples
- `try-runtime` being used in [FRAME's Staking pallet][staking-frame].
